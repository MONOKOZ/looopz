...
.main-container {
   padding: 80px var(--space-4) calc(120px + env(safe-area-inset-bottom));
   max-width: 600px;
   margin: 0 auto;
   min-height: 100vh;
   min-height: 100dvh;
...
/* Show loop handles only when loop is enabled */
.loop-handle.show {
   visibility: visible;
}
...
@media (max-width: 480px) {
   .main-container {
       padding: 80px var(--space-3) calc(120px + env(safe-area-inset-bottom));
   }
